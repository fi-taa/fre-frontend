...
export function useDashboardData(departmentId: number): UseDashboardDataReturn {
  const { data: students = [], isLoading } = useListStudentsQuery({ department_id: departmentId }, { skip: departmentId <= 0 });
  ...
  const records: PersonRecord[] = useMemo(() => {
    return students.map(studentToRecordView);
  }, [students]);
